...
def manager_salary(tickets: List[List[Ticket]], is_regional: bool) -> Tuple[float, float]:
    ...
    if sales_sum > manager_weekly_sales['rate4']:
        ...
        today_salary = ceil(sales_number[-1] * manager_ticket_payments['rate4'] * 100) / 100
    elif sales_sum > manager_weekly_sales['rate3']:
        week_salary = ceil(sales_sum * manager_ticket_payments['rate3'] * 100) / 100
        today_salary = ceil(sales_number[-1] * manager_ticket_payments['rate3'] * 100) / 100
    elif sales_sum > manager_weekly_sales['rate2']:
        week_salary = ceil(sales_sum * manager_ticket_payments['rate2'] * 100) / 100
        today_salary = ceil(sales_number[-1] * manager_ticket_payments['rate2'] * 100) / 100
    elif sales_sum > manager_weekly_sales['rate1']:
        week_salary = ceil(sales_sum * manager_ticket_payments['rate1'] * 100) / 100
        today_salary = ceil(sales_number[-1] * manager_ticket_payments['rate1'] * 100) / 100
    else:
        week_salary = manager_set_salary
        today_salary = ceil(sales_number[-1] * manager_ticket_payments['rate0'] * 100) / 100
...
def agent_salary(tickets: List[List[Ticket]], is_regional: bool, is_new: bool) -> Tuple[float, float]:
    sales_number = apply_coefficient(tickets)  # sales*coefficients by day
    ...
    if min_sales >= AGENT_RATES['rate2']:
        ...
        today_salary = ceil(sales_number[-1] * agent_ticket_payments['rate2'] * 100) / 100
    elif min_sales < AGENT_RATES['rate1']:
        week_salary = ceil(sales_sum * agent_ticket_payments['rate0'] * 100) / 100
        today_salary = ceil(sales_number[-1] * agent_ticket_payments['rate0'] * 100) / 100
    else:
        week_salary = ceil(sales_sum * agent_ticket_payments['rate1'] * 100) / 100
        today_salary = ceil(sales_number[-1] * agent_ticket_payments['rate1'] * 100) / 100

    return week_salary, today_salary


def agent_week_salary(tickets: List[List[Ticket]], is_regional: bool, is_new: bool) -> float:
    return agent_salary(tickets, is_regional, is_new)[0]


def agent_day_salary(tickets: List[List[Ticket]], is_regional: bool, is_new: bool) -> float:
    return agent_salary(tickets, is_regional, is_new)[1]
